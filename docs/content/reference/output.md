...
describes what users get. Pick a format with `-o`, or let bsky choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
bsky <command> -o table   # aligned columns for reading
bsky <command> -o jsonl   # one JSON object per line, for piping
bsky <command> -o json    # a single JSON array
bsky <command> -o csv     # spreadsheet friendly
bsky <command> -o tsv     # tab-separated
bsky <command> -o url     # just the URL column
bsky <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
bsky <command> --fields id,title,url
```
...
```bash
bsky <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
bsky <command>            # a table, because this is a terminal
bsky <command> | wc -l    # JSONL, because this is a pipe
```
