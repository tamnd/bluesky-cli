// Package cli builds the bsky command tree on top of the bluesky library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: bluesky.DefaultConfig()}

	root := &cobra.Command{
		Use:   "bsky",
		Short: "A command line for Bluesky.",
		Long: `bsky reads public Bluesky data through the AT Protocol AppView API.
No key or account is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

bsky is an independent tool and is not affiliated with, endorsed by, or
sponsored by Bluesky Social PBC or the AT Protocol project.`,
		SilenceUsage:  true,
		SilenceErrors: true,
	...
	pf.StringVar(&app.cfg.BaseURL, "base-url", app.cfg.BaseURL, "Bluesky AppView base URL")
...
func (a *App) setup() error {
	...
	a.client = bluesky.NewClient(a.cfg)
	return nil
}
